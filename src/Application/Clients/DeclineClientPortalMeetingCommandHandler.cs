...
public sealed class DeclineClientPortalMeetingCommandHandler
    : IRequestHandler<DeclineClientPortalMeetingCommand, Result>
{
    private static readonly Error MeetingNotFoundError = new("Meetings.NotFound", "Meeting was not found.", ErrorType.NotFound);
    private static readonly Error MeetingInvalidStateError = new("Meetings.InvalidState", "Meeting cannot be declined in its current state.", ErrorType.Conflict);

    private readonly ICurrentClientResolver _currentClientResolver;
    private readonly IMeetingRepository _meetingRepository;
    private readonly IUnitOfWork _unitOfWork;

    public DeclineClientPortalMeetingCommandHandler(
    ...
    public async Task<Result> Handle(DeclineClientPortalMeetingCommand request, CancellationToken cancellationToken)
    {
