...
@Component({
  selector: 'app-accept-staff-invitation-screen',
  standalone: true,
  ...
  template: `
    <main class="min-h-screen bg-muted/30 px-4 py-8 sm:px-6">
      <ui-card class="mx-auto w-full max-w-md">
        <ui-card-header>
          <ui-card-title class="text-xl">Accept team invitation</ui-card-title>
          <ui-card-description>
            Set your account password to join the business team.
          </ui-card-description>
        </ui-card-header>
...
export class AcceptStaffInvitationScreenComponent {
  ...
  async submit(): Promise<void> {
    ...
    try {
      await firstValueFrom(
        this.authApiService.acceptStaffInvitation({
          token: this.token,
