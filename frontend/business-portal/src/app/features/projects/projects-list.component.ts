...
type SortKey = 'name' | 'client' | 'status' | 'budget' | 'startDate' | 'endDate';
      ...
      <header class="flex items-center justify-between gap-4 pb-5">
        <div>
          ...
          <p class="mt-1 text-sm text-muted-foreground">
            Browse and manage your project catalog.
          </p>
        </div>
            ...
            @if (projectStore.isLoading()) {
              ...
              @for (project of displayedProjects(); track project.id) {
                <tr class="border-b border-border/50">
                  ...
                  <td class="px-3 py-4">
                    <span class="inline-flex rounded-md bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-700">
                      {{ project.clientCompanyName }}
                    </span>
                  </td>
                  <td class="px-3 py-4">
